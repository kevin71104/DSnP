...
char mygetc(istream& istr)
{
   char ch;
   set_keypress();
   istr.unsetf(ios_base::skipws);//deactivate skipws
   istr >> ch;
   istr.setf(ios_base::skipws);//activate skipws
   reset_keypress();
   #ifdef TEST_ASC	//only do line53 when running testasc.cpp
...
getChar(istream& istr)
{
   ...
   switch (ch) {
      ...
      // For the remaining printable and undefined keys
      default:
         if (isprint(ch)) return returnCh(ch);	//isprint(c):if c is printable return 1 ,else return 0
         else return returnCh(UNDEFINED_KEY);
   }
