   ...
   if(myStrNCmp("-Row",options[0],2)==0)doRow=true;
	...
	for(size_t i=1;i<options.size();i++){
		if(options[i]=="-")
			...
			cerr << "Error: " << options[i] << " is not an integer!!\n";
			return CMD_EXEC_ERROR;
		}
	}
	if(data.empty())	
...
{  
   ...
   if (!dbtbl) {
	...
	bool dorow=false,docol=false,dotab=false,dosum=false;
	for(size_t i=0;i<options.size();i++)
	{
		if(myStrNCmp("-Row",options[i],2) == 0){
			if(spec.size())
		...
		else if(myStrNCmp("-Column",options[i],2) == 0 ){
			if(spec.size())
				return CmdExec::errorOption(CMD_OPT_ILLEGAL,options[i]);
			if(rowidx!=(-1))
				return CmdExec::errorOption(CMD_OPT_ILLEGAL,options[i]);
		...
		else if(myStrNCmp("-Table",options[i],2) == 0 ){
			if(spec.size())
				return CmdExec::errorOption(CMD_OPT_ILLEGAL,options[i]);
			if(rowidx!=(-1))
				return CmdExec::errorOption(CMD_OPT_ILLEGAL,options[i]);
			...
			if(dotab)
				...
				if(rowidx==-1){
					if(! checkRowIdx(options[i],rowidx)) return CMD_EXEC_ERROR;
				}
				else return CmdExec::errorOption(CMD_OPT_EXTRA,options[i]);
			}
			else if(docol){
				if(colidx==-1){
					if(! checkColIdx(options[i],colidx)) return CMD_EXEC_ERROR;
				}
				else return CmdExec::errorOption(CMD_OPT_EXTRA,options[i]);
			}
			else if(rowidx==-1){
