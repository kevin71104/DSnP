...
CmdParser::execOneCmd()
{
   ...
   if (_dofile != 0)
      ...
      newCmd = readCmd(cin);
	//when push newline key readcmd will return true
   // execute the command
   if (newCmd) {
      string option;
...
CmdParser::parseCmd(string& option)
{
   ...
   CmdExec* e=0;
   string cmd;
   size_t start=myStrGetTok(str,cmd,0,char(32));
   start = str.find_first_not_of(char(32), start); 
   e=getCmd(cmd);
   if(!e){
		cerr << "Error: Illegal option!! (" << cmd << ")" << endl;
		return 0;
	}
   else{
...
CmdParser::listCmd(const string& str)
{
   ...
   string cmd;
	myStrGetTok(str,cmd);
   vector<CmdMap::const_iterator> match;
	if(cmd.empty()){
		int times=0;
		for(CmdMap::const_iterator iter=_cmdMap.begin();iter!=_cmdMap.end();iter++){
	...
	for(CmdMap::const_iterator iter=_cmdMap.begin();iter!=_cmdMap.end();iter++){
		string library=(iter->first)+(iter->second)->getOptCmd();
		if(myStrNCmp(library,cmd,1)==0)
	...
	if(match.size()>1){
		for(size_t i=0;i<match.size();i++){
			if(! (i%5))cout<<endl;
			cout<< setw(12)<< left <<(match[i]->first)+(match[i]->second)->getOptCmd();	
		}
		cout<<endl;
		//cmdMgr->printPrompt();
		printPrompt();
	...
	if(match.size()==0)
		mybeep();
	if(match.size()==1){
		string library=(match[0]->first)+(match[0]->second)->getOptCmd();
		if(str.size()<=cmd.size()){
			for(size_t i=cmd.size();i<library.size();i++)
				insertChar(library[i]);
			insertChar(char(32));
		}
		else{
			cout<<endl;
			(match[0]->second)->usage(cout);
			//cmdMgr->printPrompt();
			printPrompt();
			for(int i=0;i<_readBufEnd-_readBuf;i++)
				cout<<*(_readBuf+i);
...
CmdParser::getCmd(string cmd)
{
   ...
   // TODO...
	for(CmdMap::const_iterator iter=_cmdMap.begin();iter!=_cmdMap.end();iter++){
		...
		CmdExec* exe = iter->second;
		str = str+exe->getOptCmd();
		//cout<<"enter"<<i<<cmd<<" "<<str<<" "<<len<<endl;
		if(myStrNCmp(str,cmd,len) == 0){
			e=exe;
			break;
		}
	}
...
// i.e. there should have other specification if "optional" is false
...
(const string& option, string& token, bool optional) const
{
   ...
   if (n != string::npos) {
      errorOption(CMD_OPT_EXTRA, option.substr(n+1));
      return false;
   }
   return true;
}
